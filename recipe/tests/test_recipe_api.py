...
def detail_url(recipe_id):
    """Возвращает URL для просмотра рецепта"""
    return reverse('recipe:recipe-detail', args=[recipe_id])


def sample_tag(user, name='Основное блюдо'):
    """Создает и возвращает экземляр тега"""
    return Tag.objects.create(user=user, name=name)


def sample_ingredient(user, name='Корица'):
    """Создает и возвращает экземляр игридиета"""
    return Ingredient.objects.create(user=user, name=name)


def sample_recipe(user, **params):
    """Создает и возвращает экземпляр рецепта"""
    defaults = {
...
class PublicRecipeApiTests(TestCase):
    """Тест апи неавторизованного пользователя"""
...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_recipes_limited_to_user(self):
        """Tест на вывод рецептов пользователя"""
    ...
    def test_view_recipe_detail(self):
        """Тестовый просмотр сведений о рецепте"""
    ...
    def test_create_recipe_with_ingredients(self):
        """Тест на создание рецепта с ингридиентами"""
    ...
    def test_full_update_recipe(self):
        """Тест на обновление рецепта методом Put"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
